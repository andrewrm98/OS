...
typedef struct{
	...
	unsigned char page1Use;
	unsigned char page2Use;
	unsigned char page3Use;
	unsigned char page4Use;
} freeL;
...
void masterFunction (int process, char * instr, int addr, int val) 
{
	...
	else if (!(strcmp(instr, "print"))) 	{ printMem(); }
...
int getTable(int pid)
{
	...
	{
		int freeSpot = findFreePages((pid * -1), 0, 0); 							//returns where to put the table
...
int getFrame(int tableFrame, int addr, int process)
{
	...
	{
		memPage putInMem;
		freeSpot = findFreePages(process, addr);
		for(int i =0; i < 16; i++){
			putInMem.pos[i] = -1;
		}
...
void printMem()
{
	...
	{
		printf("Frame %d: ", i);
		for(int j = 0; j < 16; j++) { printf("%d ", memory[(16*i) + j]); }
	}
	printf("\nPage Tables: ");
	for(int i = 0; i < 4; i++) { printf("Page %d: %d ", i, tableLocs[i]); }
	printf("\n");
}
...
int swapOut(int evicter) 
{
	...
	FILE* hdd;
	hdd = fopen("hdd.txt", "a");
	fprintf(hdd, "%d", memory[(evicter*16)]);
	for(int i = 1; i < 16; i++) { fprintf(hdd, " %d", memory[(evicter*16) + i]); }
	fprintf(hdd, "\n");
	fclose(hdd);
	if(isT) 																	//if its a table
	{
		...
		{
			if(tableLocs[i] == evicter) { tableLocs[evicter] = (-1*hddLine); } //update its location marking it as in disk
		}
	}
			...
			if(tableLocs[i] >= 0) 												//if table i is present
			{
				for(int j = 0; j < 4; j++) 										//go through its memPages
				{
					if(memory[((16*i) + (4*j) + 3)] == evicter) 				//if its memPage j's frame is the one getting evicted
					{
						...
						memory[((16*i) + (4*j) + 3)] = hddLine;					//update its frame bit
					}
				}
			}
		}
	}
	printf("Swapped out page %d to hdd line: %d\n", evicter, hddLine);
	hddLine++;
	return hddLine;
}
...
int swapIn(int whereIsIt, int type, int addr)
{
	int freeSpot = findFreePages(type, addr);
	int actLoc = whereIsIt - 1;
	memPage reader;
	for(int i = 0; i < 16; i++){ reader.pos[i] = 0; }
	char ln[255][100];
	FILE *hdd = fopen("hdd.txt", "r");
	if(hdd != NULL)
	{
		for(int i = 0; i < 255; i++)
		{
			if(fgets(ln[i], sizeof(ln[i]), hdd) == NULL) { i = 255; }
		}
		char temp[4];
		int posI = 0;
		int index = 0;
	...
	printf("Swapped into page %d from hdd line: %d\n", freeSpot, whereIsIt);
